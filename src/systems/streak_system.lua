...
-- Font cache for performance (LRU with size limit)
StreakSystem.fontCache = {}
StreakSystem.fontCacheOrder = {} -- Track access order
StreakSystem.MAX_FONT_CACHE_SIZE = 10
...
-- Get cached font for performance with error handling (LRU implementation)
function StreakSystem.getFont(size)
    if not size or size <= 0 then
        size = 12 -- Default font size
    end
    -- Check if font exists in cache
    if StreakSystem.fontCache[size] then
        -- Update access order (move to end)
        for i, s in ipairs(StreakSystem.fontCacheOrder) do
            if s == size then
                table.remove(StreakSystem.fontCacheOrder, i)
                break
            end
        end
        table.insert(StreakSystem.fontCacheOrder, size)
        return StreakSystem.fontCache[size]
    end
    -- Evict oldest font if cache is full
    if #StreakSystem.fontCacheOrder >= StreakSystem.MAX_FONT_CACHE_SIZE then
        local oldestSize = table.remove(StreakSystem.fontCacheOrder, 1)
        StreakSystem.fontCache[oldestSize] = nil
    end
    -- Create new font
