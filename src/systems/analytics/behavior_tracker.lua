...
function BehaviorTracker.analyzeExplorationStyle(planet, discoveryMethod, attempts)
    local exploration = BehaviorTracker.explorationProfile
    -- Update planet visit distribution with memory limits
    ...
    -- Limit tracked planets to prevent memory growth
    local MAX_TRACKED_PLANETS = 200
    local planetCount = 0
    for _ in pairs(exploration.planetVisitDistribution) do
        planetCount = planetCount + 1
    end
    if planetCount > MAX_TRACKED_PLANETS then
        -- Remove least visited planets
        local planets = {}
        for id, visits in pairs(exploration.planetVisitDistribution) do
            table.insert(planets, {id = id, visits = visits})
        end
        table.sort(planets, function(a, b) return a.visits < b.visits end)
        -- Remove bottom 25% of least visited
        local toRemove = math.floor(planetCount * 0.25)
        for i = 1, toRemove do
            exploration.planetVisitDistribution[planets[i].id] = nil
        end
    end
