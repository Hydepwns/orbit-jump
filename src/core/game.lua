...
local PerformanceOverlay = Utils.require("src.systems.performance_overlay")
local HotReload = Utils.require("src.utils.hot_reload")
local Game = {}
--[[
    ═══════════════════════════════════════════════════════════════════════════
...
function Game.initGraphics()
    ...
    local fontError = nil
    local regularFont = Utils.ErrorHandler.safeCall(function()
        return love.graphics.newFont("assets/fonts/MonaspaceArgon-Regular.otf", 16)
    end, {
    ...
    if regularFont then
        ...
        local fallbackFont = Utils.ErrorHandler.safeCall(function()
...
function Game.initSystems()
    ...
    -- Initialize performance overlay (F3 to toggle)
    if PerformanceOverlay then
        PerformanceOverlay.init()
        Game.performanceOverlay = PerformanceOverlay
        -- Enable by default in development mode
        if Config.debug and Config.debug.enabled then
            PerformanceOverlay.config.enabled = true
        end
    end
    -- Initialize hot reload system (F5 to reload)
    if HotReload and Config.debug and Config.debug.enabled then
        HotReload.init()
        HotReload.setEnabled(true)
        Game.hotReload = HotReload
        Utils.Logger.info("Hot reload enabled - Press F5 to reload changed modules")
    end
    -- Initialize audio
...
function Game.update(dt)
    ...
    if Game.performanceOverlay then
        Game.performanceOverlay.update(dt)
    end
    -- Update hot reload system
    if Game.hotReload then
...
function Game.draw()
    ...
    -- Draw performance overlay (always on top)
    if Game.performanceOverlay then
        Game.performanceOverlay.draw()
    end
end
function Game.handleKeyPress(key)
    ...
    -- Handle performance overlay toggle
    if Game.performanceOverlay and Game.performanceOverlay.keypressed then
        Game.performanceOverlay.keypressed(key)
    end
    -- Handle hot reload
    if Game.hotReload and Game.hotReload.keypressed then
        Game.hotReload.keypressed(key)
    end
...
function Game.cleanupSystemHealth()
    --[[Prevent unbounded growth of system health tables--]]
    local MAX_ENTRIES = 100
    -- Cleanup critical systems table
    if systemHealth.criticalSystems then
        local count = 0
        for _ in pairs(systemHealth.criticalSystems) do
            count = count + 1
        end
        if count > MAX_ENTRIES then
            -- Keep only the most recent entries
            local keys = {}
            for k in pairs(systemHealth.criticalSystems) do
                table.insert(keys, k)
            end
            -- Remove oldest entries (assuming keys are added chronologically)
            for i = 1, count - MAX_ENTRIES do
                systemHealth.criticalSystems[keys[i]] = nil
            end
        end
    end
    -- Cleanup recovery attempts
    if systemHealth.recoveryAttempts then
        local count = 0
        for _ in pairs(systemHealth.recoveryAttempts) do
            count = count + 1
        end
        if count > MAX_ENTRIES then
            -- Keep only the most recent attempts
            local keys = {}
            for k in pairs(systemHealth.recoveryAttempts) do
                table.insert(keys, k)
            end
            for i = 1, count - MAX_ENTRIES do
                systemHealth.recoveryAttempts[keys[i]] = nil
            end
        end
    end
end
function Game.updatePerformanceMetrics(frameTime)
    ...
    -- Periodically cleanup system health tables
    if love.timer.getTime() % 60 < frameTime then -- Once per minute
        Game.cleanupSystemHealth()
    end
end
