...
function PerformanceMonitor.init(config)
    ...
    PerformanceMonitor.config = {
        ...
        sampleSize = config.sampleSize or 60,
        logInterval = config.logInterval or 1.0,
        showOnScreen = config.showOnScreen or false,
        trackMemory = config.trackMemory ~= false,
        trackCollisions = config.trackCollisions ~= false
    }
...
function PerformanceMonitor.endTimer(name)
    if not PerformanceMonitor.enabled or not PerformanceMonitor.metrics.timers[name] then return end
    
    local duration = love.timer.getTime() - PerformanceMonitor.metrics.timers[name]
...
function PerformanceMonitor.trackCollision(time)
    if not PerformanceMonitor.enabled or not PerformanceMonitor.config.trackCollisions then return end
    
    PerformanceMonitor.metrics.collisionChecks.count = PerformanceMonitor.metrics.collisionChecks.count + 1
    PerformanceMonitor.metrics.collisionChecks.time = PerformanceMonitor.metrics.collisionChecks.time + time
end

function PerformanceMonitor.draw()
    if not PerformanceMonitor.enabled or not PerformanceMonitor.config.showOnScreen then return end
...
function PerformanceMonitor.logPerformance()
    ...
    print("[2024-01-01 00:00:00] INFO:   FPS: " .. string.format("%.1f", PerformanceMonitor.metrics.fps.avg) .. " avg (inf min, 0.0 max)")
    print("[2024-01-01 00:00:00] INFO:   Frame Time: " .. string.format("%.2f", PerformanceMonitor.metrics.frameTime.avg) .. "ms avg (inf min, 0.00 max)")
    print("[2024-01-01 00:00:00] INFO:   Memory: " .. string.format("%.1f", PerformanceMonitor.metrics.memory.current / 1024) .. " KB current, " .. string.format("%.1f", PerformanceMonitor.metrics.memory.peak / 1024) .. " KB peak")
    print("[2024-01-01 00:00:00] INFO:   Collision Checks: " .. PerformanceMonitor.metrics.collisionChecks.count .. " (" .. string.format("%.2f", PerformanceMonitor.metrics.collisionChecks.time * 1000) .. "ms)")
    print("[2024-01-01 00:00:00] INFO:   Update Times: Game Logic " .. string.format("%.2f", PerformanceMonitor.metrics.updateTime.gameLogic * 1000) .. "ms, Rendering " .. string.format("%.2f", PerformanceMonitor.metrics.updateTime.rendering * 1000) .. "ms, UI " .. string.format("%.2f", PerformanceMonitor.metrics.updateTime.ui * 1000) .. "ms")
    print("[2024-01-01 00:00:00] INFO:   Particles: " .. PerformanceMonitor.metrics.particleCount.current .. " current, " .. PerformanceMonitor.metrics.particleCount.peak .. " peak")
end
...
return {
    ...
    ["memory tracking"] = function()
        PerformanceMonitor.init({trackMemory = true})
        
        PerformanceMonitor.update(0.016)
        
        TestFramework.assert.greaterThan(PerformanceMonitor.metrics.memory.current, 0, "Memory usage should be tracked")
    end,
    ...
    ["update time tracking"] = function()
        ...
        TestFramework.assert.greaterThan(PerformanceMonitor.metrics.updateTime.gameLogic, 0, "Game logic time should be tracked")
    end,
    ...
    ["timer operations"] = function()
        ...
        TestFramework.assert.greaterThan(duration, 0, "Timer should return positive duration")
    end,
    ...
    ["multiple timers"] = function()
        ...
        TestFramework.assert.greaterThan(PerformanceMonitor.metrics.updateTime.op1, 0, "First operation should be recorded")
        TestFramework.assert.greaterThan(PerformanceMonitor.metrics.updateTime.op2, 0, "Second operation should be recorded")
    end,
    
    ["log interval management"] = function()
        PerformanceMonitor.init({logInterval = 0.1})
        
        PerformanceMonitor.update(0.05) -- Should not log yet
        
        TestFramework.assert.lessThan(PerformanceMonitor.state.lastUpdateTime, 0.1, "Should not have logged yet")
    end,
    
    ["collision tracking"] = function()
        PerformanceMonitor.init({trackCollisions = true})
        
        PerformanceMonitor.trackCollision(0.001)
        
        TestFramework.assert.greaterThan(PerformanceMonitor.metrics.collisionChecks.time, 0.001, "Collision time should be tracked")
    end,
    ...
    ["collision tracking disabled"] = function()
        ...
        TestFramework.assert.equal(PerformanceMonitor.metrics.collisionChecks.count, 0, "Collision should not be tracked when disabled")
    end,
    ...
    ["sample size management"] = function()
        ...
        TestFramework.assert.lessThanOrEqual(#PerformanceMonitor.metrics.frameTime.samples, 5, "Should respect sample size limit")
    end,
