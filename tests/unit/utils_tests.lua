...
local tests = {
    ...
    ["should create object pool"] = function()
        ...
        local pool = Utils.ObjectPool.new(createFunc, 10)
    ...
    ["should handle safe function calls with errors"] = function()
        local success, error = Utils.ErrorHandler.safeCall(function() error("test error") end)
        ModernTestFramework.assert.isFalse(success, "Should fail for error function")
        ModernTestFramework.assert.notNil(error, "Should return error message")
    end,
