...
local tests = {
    ["test sine wave generation"] = function()
        ...
        TestFramework.utils.assertNotNil(soundData, "Should generate sound data")
        TestFramework.utils.assertEqual(4410, soundData.samples, "Should have correct number of samples")
        TestFramework.utils.assertEqual(44100, soundData.rate, "Should have correct sample rate")
        ...
        TestFramework.utils.assertTrue(hasNonZeroSamples, "Should have non-zero samples")
    end,
    ...
    ["test square wave generation"] = function()
        ...
        for i = 0, 100 do
            ...
            if sample and sample > 0 then
                positiveFound = true
                TestFramework.utils.assertTrue(math.abs(sample - 0.3) < 0.001, "Positive should be 0.3")
            elseif sample and sample < 0 then
                negativeFound = true
                TestFramework.utils.assertTrue(math.abs(sample + 0.3) < 0.001, "Negative should be -0.3")
            end
        end
        TestFramework.utils.assertTrue(positiveFound and negativeFound, "Should have both positive and negative values")
    end,
    ...
    ["test noise generation"] = function()
        ...
        TestFramework.utils.assertTrue(count > 1, "Noise should have varied values")
    ...
    ["test envelope application"] = function()
        ...
        TestFramework.utils.assertTrue(modified, "Envelope should modify samples")
        
        -- Check attack phase (should start at 0)
        local firstSample = math.abs(mockSamples[0] or 0)
        TestFramework.utils.assertTrue(firstSample < 0.1, "First sample should be near zero")
    end,
    
    ["test game sounds generation"] = function()
        local sounds = SoundGenerator.generateGameSounds()
        
        TestFramework.utils.assertNotNil(sounds, "Should generate sounds table")
        TestFramework.utils.assertNotNil(sounds.jump, "Should have jump sound")
        TestFramework.utils.assertNotNil(sounds.ring, "Should have ring sound")
        TestFramework.utils.assertNotNil(sounds.dash, "Should have dash sound")
        TestFramework.utils.assertNotNil(sounds.land, "Should have land sound")
        TestFramework.utils.assertNotNil(sounds.combo, "Should have combo sound")
        TestFramework.utils.assertNotNil(sounds.gameOver, "Should have game over sound")
        TestFramework.utils.assertNotNil(sounds.ambient, "Should have ambient sound")
        
        -- Check ambient sound properties
        TestFramework.utils.assertTrue(sounds.ambient.looping, "Ambient should loop")
        TestFramework.utils.assertEqual(0.2, sounds.ambient.volume, "Ambient volume should be 0.2")
        
        -- Check that all sounds have the correct type
        TestFramework.utils.assertEqual("static", sounds.jump.type, "Jump should be static")
        TestFramework.utils.assertEqual("stream", sounds.ambient.type, "Ambient should be stream")
    end,
    ...
    ["test jump sound characteristics"] = function()
        ...
        TestFramework.utils.assertNotNil(sounds.jump.soundData, "Jump should have sound data")
        ...
        TestFramework.utils.assertTrue(hasNonZeroSamples, "Jump should have non-zero samples")
    end,
    ...
    ["test ring sound harmonics"] = function()
        ...
        TestFramework.utils.assertNotNil(sounds.ring.soundData, "Ring should have sound data")
        ...
        TestFramework.utils.assertTrue(sampleCount > 0, "Ring should have samples")
    end,
    ...
    ["test sound data parameters"] = function()
        ...
        for i, test in ipairs(testCases) do
            ...
            TestFramework.utils.assertEqual(expectedSamples, soundData.samples, 
                "Test " .. i .. " should have correct samples")
            TestFramework.utils.assertEqual(test.rate, soundData.rate, 
    ...
    ["test envelope edge cases"] = function()
        ...
        TestFramework.utils.assertTrue(success, "Should handle short envelope times")
        
        -- Test with zero times
        success = pcall(function()
            SoundGenerator.applyEnvelope(soundData, 0, 0, 1.0, 0)
        end)
        TestFramework.utils.assertTrue(success, "Should handle zero envelope times")
    end,
    ...
    ["test all sound types created"] = function()
        ...
        for _, soundType in ipairs(soundTypes) do
            TestFramework.utils.assertNotNil(sounds[soundType], soundType .. " sound should exist")
            TestFramework.utils.assertNotNil(sounds[soundType].soundData, 
    ...
    ["test default sample rate"] = function()
        ...
        TestFramework.utils.assertEqual(44100, soundData.rate, "Should use default sample rate of 44100")
        
        soundData = SoundGenerator.generateSquareWave(440, 0.1)
        TestFramework.utils.assertEqual(44100, soundData.rate, "Square wave should use default sample rate")
        
        soundData = SoundGenerator.generateNoise(0.1)
        TestFramework.utils.assertEqual(44100, soundData.rate, "Noise should use default sample rate")
    end,
    ...
    ["test sample clamping"] = function()
        ...
        TestFramework.utils.assertTrue(allInRange, "All samples should be in range [-1, 1]")
    end
}

-- Run the test suite
local function run()
    return TestFramework.runSuite("Sound Generator Tests", tests)
end
